library(ggplot2)
f75 <- read.csv("../data/output/paul_freq/75_frequency.csv", header=TRUE, sep="\t")
particles <- unique(f75$particle)
filename <- paste0("../data/output/paul_R/plots/f75_par1_",particles[1],".png")
png(filename)
par1 <- f75[which(f75$particle==particles[1]),]
...
particles <- unique(f75$particle)
filename <- paste0("../data/output/paul_R/plots/f75_par2_",particles[2],".png")
png(filename)
par2 <- f75[which(f75$particle==particles[2]),]
...
particles <- unique(f75$particle)
filename <- paste0("../data/output/paul_R/plots/f75_par3_",particles[3],".png")
png(filename)
par3 <- f75[which(f75$particle==particles[3]),]
...
particles <- unique(f75$particle)
filename <- paste0("../data/output/paul_R/plots/f75_par4_",particles[4],".png")
png(filename)
par4 <- f75[which(f75$particle==particles[4]),]
