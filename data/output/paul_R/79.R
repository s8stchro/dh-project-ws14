library(ggplot2)
f79 <- read.csv("../data/output/paul_freq/79_frequency.csv", header=TRUE, sep="\t")
particles <- unique(f79$particle)
filename <- paste0("../data/output/paul_R/plots/f79_par1_",particles[1],".png")
png(filename)
par1 <- f79[which(f79$particle==particles[1]),]
...
particles <- unique(f79$particle)
filename <- paste0("../data/output/paul_R/plots/f79_par2_",particles[2],".png")
png(filename)
par2 <- f79[which(f79$particle==particles[2]),]
...
particles <- unique(f79$particle)
filename <- paste0("../data/output/paul_R/plots/f79_par3_",particles[3],".png")
png(filename)
par3 <- f79[which(f79$particle==particles[3]),]
...
particles <- unique(f79$particle)
filename <- paste0("../data/output/paul_R/plots/f79_par4_",particles[4],".png")
png(filename)
par4 <- f79[which(f79$particle==particles[4]),]
...
particles <- unique(f79$particle)
filename <- paste0("../data/output/paul_R/plots/f79_par5_",particles[5],".png")
png(filename)
par5 <- f79[which(f79$particle==particles[5]),]
...
particles <- unique(f79$particle)
filename <- paste0("../data/output/paul_R/plots/f79_par6_",particles[6],".png")
png(filename)
par6 <- f79[which(f79$particle==particles[6]),]
...
particles <- unique(f79$particle)
filename <- paste0("../data/output/paul_R/plots/f79_par7_",particles[7],".png")
png(filename)
par7 <- f79[which(f79$particle==particles[7]),]
...
particles <- unique(f79$particle)
filename <- paste0("../data/output/paul_R/plots/f79_par8_",particles[8],".png")
png(filename)
par8 <- f79[which(f79$particle==particles[8]),]
...
particles <- unique(f79$particle)
filename <- paste0("../data/output/paul_R/plots/f79_par9_",particles[9],".png")
png(filename)
par9 <- f79[which(f79$particle==particles[9]),]
par9$perc_label[par9$perc_label == 0] <- NA
par9_count = sum(par9$freq_t)
ymax <- max(par9$freq_t)
ymax_s = ymax*1.1-ymax
title <- paste(par9$particle[9], " (total_count = ", par9_count,", letter =", par9$letter[9],")" )
p_freq_t <- barplot(par9$freq_t, main=title, xlab="Position", ylab= "Frequency" ,col=c("cyan4"), names.arg=par9$position,ylim=c(0,ymax*1.2))
p_freq_s<- barplot(par9$freq_s, col=c("darkgoldenrod1"),add=T)
legend("topright", inset=.05, c("position_from_pip","position_from_start"), fill=c("cyan4","darkgoldenrod1"))
text(p_freq_t, par9$freq_t-0.5, labels= par9$freq_is, col="white")
text(p_freq_s, par9$freq_s-0.5, labels=par9$freq_s, col="black")
text(p_freq_t, par9$freq_t+ymax_s, labels=par9$perc_label, col="blue",srt=90)

particles <- unique(f79$particle)
filename <- paste0("../data/output/paul_R/plots/f79_par10_",particles[10],".png")
png(filename)
par10 <- f79[which(f79$particle==particles[10]),]
par10$perc_label[par10$perc_label == 0] <- NA
par10_count = sum(par10$freq_t)
ymax <- max(par10$freq_t)
ymax_s = ymax*1.1-ymax
title <- paste(par10$particle[10], " (total_count = ", par10_count,", letter =", par10$letter[10],")" )
p_freq_t <- barplot(par10$freq_t, main=title, xlab="Position", ylab= "Frequency" ,col=c("cyan4"), names.arg=par10$position,ylim=c(0,ymax*1.2))
p_freq_s<- barplot(par10$freq_s, col=c("darkgoldenrod1"),add=T)
legend("topright", inset=.05, c("position_from_pip","position_from_start"), fill=c("cyan4","darkgoldenrod1"))
text(p_freq_t, par10$freq_t-0.5, labels= par10$freq_is, col="white")
text(p_freq_s, par10$freq_s-0.5, labels=par10$freq_s, col="black")
text(p_freq_t, par10$freq_t+ymax_s, labels=par10$perc_label, col="blue",srt=90)

particles <- unique(f79$particle)
filename <- paste0("../data/output/paul_R/plots/f79_par11_",particles[11],".png")
png(filename)
par11 <- f79[which(f79$particle==particles[11]),]
par11$perc_label[par11$perc_label == 0] <- NA
par11_count = sum(par11$freq_t)
ymax <- max(par11$freq_t)
ymax_s = ymax*1.1-ymax
title <- paste(par11$particle[11], " (total_count = ", par11_count,", letter =", par11$letter[11],")" )
p_freq_t <- barplot(par11$freq_t, main=title, xlab="Position", ylab= "Frequency" ,col=c("cyan4"), names.arg=par11$position,ylim=c(0,ymax*1.2))
p_freq_s<- barplot(par11$freq_s, col=c("darkgoldenrod1"),add=T)
legend("topright", inset=.05, c("position_from_pip","position_from_start"), fill=c("cyan4","darkgoldenrod1"))
text(p_freq_t, par11$freq_t-0.5, labels= par11$freq_is, col="white")
text(p_freq_s, par11$freq_s-0.5, labels=par11$freq_s, col="black")
text(p_freq_t, par11$freq_t+ymax_s, labels=par11$perc_label, col="blue",srt=90)
