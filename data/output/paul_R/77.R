library(ggplot2)
f77 <- read.csv("../data/output/paul_freq/77_frequency.csv", header=TRUE, sep="\t")
particles <- unique(f77$particle)
filename <- paste0("../data/output/paul_R/plots/f77_par1_",particles[1],".png")
png(filename)
par1 <- f77[which(f77$particle==particles[1]),]
...
particles <- unique(f77$particle)
filename <- paste0("../data/output/paul_R/plots/f77_par2_",particles[2],".png")
png(filename)
par2 <- f77[which(f77$particle==particles[2]),]
...
particles <- unique(f77$particle)
filename <- paste0("../data/output/paul_R/plots/f77_par3_",particles[3],".png")
png(filename)
par3 <- f77[which(f77$particle==particles[3]),]
