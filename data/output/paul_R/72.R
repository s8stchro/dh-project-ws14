library(ggplot2)
f72 <- read.csv("../data/output/paul_freq/72_frequency.csv", header=TRUE, sep="\t")
particles <- unique(f72$particle)
filename <- paste0("../data/output/paul_R/plots/f72_par1_",particles[1],".png")
png(filename)
par1 <- f72[which(f72$particle==particles[1]),]
...
particles <- unique(f72$particle)
filename <- paste0("../data/output/paul_R/plots/f72_par2_",particles[2],".png")
png(filename)
par2 <- f72[which(f72$particle==particles[2]),]
...
particles <- unique(f72$particle)
filename <- paste0("../data/output/paul_R/plots/f72_par3_",particles[3],".png")
png(filename)
par3 <- f72[which(f72$particle==particles[3]),]
...
particles <- unique(f72$particle)
filename <- paste0("../data/output/paul_R/plots/f72_par4_",particles[4],".png")
png(filename)
par4 <- f72[which(f72$particle==particles[4]),]
...
particles <- unique(f72$particle)
filename <- paste0("../data/output/paul_R/plots/f72_par5_",particles[5],".png")
png(filename)
par5 <- f72[which(f72$particle==particles[5]),]
