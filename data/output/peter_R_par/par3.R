library(ggplot2)
par3 <- read.csv("../../data/output/peter_freq_par/par3_frequency.csv", header=TRUE, sep="\t")
letters <- unique(par3$letter)
filename <- paste0("../../data/output/peter_R_par/plots/par3_lt82.png")
png(filename)
lt82 <- par3[which(par3$letter==82),]
...
letters <- unique(par3$letter)
filename <- paste0("../../data/output/peter_R_par/plots/par3_lt81.png")
png(filename)
lt81 <- par3[which(par3$letter==81),]
