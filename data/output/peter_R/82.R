library(ggplot2)
f82 <- read.csv("../data/output/peter_freq/82_frequency.csv", header=TRUE, sep="\t")
particles <- unique(f82$particle)
filename <- paste0("../data/output/peter_R/plots/f82_par1_",particles[1],".png")
png(filename)
par1 <- f82[which(f82$particle==particles[1]),]
...
particles <- unique(f82$particle)
filename <- paste0("../data/output/peter_R/plots/f82_par2_",particles[2],".png")
png(filename)
par2 <- f82[which(f82$particle==particles[2]),]
...
particles <- unique(f82$particle)
filename <- paste0("../data/output/peter_R/plots/f82_par3_",particles[3],".png")
png(filename)
par3 <- f82[which(f82$particle==particles[3]),]
...
particles <- unique(f82$particle)
filename <- paste0("../data/output/peter_R/plots/f82_par4_",particles[4],".png")
png(filename)
par4 <- f82[which(f82$particle==particles[4]),]
